...
export default function Visits(props: VisitProps) {
  ...
  const ContentColor =
    props.index === 0 ? colors.BG_LIGHT_PINK_COLOR : colors.BG_LIGHT_BLUE_COLOR;
  const TimeColor =
    props.index === 0 ? colors.FONT_RED_COLOR : colors.BG_DARK_BLUE_COLOR;

  const checkBooking = () =>
    state === 'Booked Now' ? setstate('Booked') : setstate('Booked Now');
